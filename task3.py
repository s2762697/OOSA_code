            ...
            if tile_midx < -1571000:
                if tile_midy > -300000:
                    if tile_midx > -1600000:
                        if tile_midy < -200000:
                            print(f'Reading file {file_id}|Tile {tile_number}')

                            # process if within bounds
                            lvis.estimateGround()
                            tiffName = f"Data2009/DEM{file_id}{tile_number}.tif"   #filename with tile identifier
                            lvis.makeDEM(command.res, tiffName)
                            tile_number +=1   # loop counter

                else:
                   print(f'Tile {tile_number} in {file_id} is not within bounds')
                   tile_number +=1
        
        except AttributeError as error:
            print(f'Tile {tile_number} in {file_id} has been skipped')
            tile_number +=1   # loop counter
