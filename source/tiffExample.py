'''Make geotiffs from an array of data, x and y for a given resolution'''
...
def writeTiff(data,x,y,res,filename="lvis_image.tif",epsg=3031):
    '''Make a geotiff from an array of points
  
    Args:
        Data = input array to be written to TIFF
        x = x coords
        y = y coords
        res = resolution of raster 
        filename = default output tiff file
        epsg = default of 3031 - the coord reference system (only just found steves bug lol)

    Returns: 
        wrrites a geotiff to file to disk
    '''
    ...
    #dst_ds.GetRasterBand(1).SetNoDataValue(0)  # set no data value
